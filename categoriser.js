...
for (let i = 0; i < maxPriority; i++) {
  priorityMsgQueues[i] = [];
  pirorityExpirationTimeQueue[i] = 5000 * (i + 1); // tiempo de expiración de cada cola comienza en 10, 20, 30 y 40 segundos
}
...
const fs = require('fs');
const logFilePathClassified = 'csv/classifier.csv';
const logFilePathREST = 'csv/classifier_REST.csv';
const logFilePathStatus = 'csv/classifier_STATUS.csv';
const logFilePathExpirations = 'csv/classifierExpirations.csv';
...
function showQueuesStatus() {
  ...
  console.log("\n-----------------------------------------------------");
  console.log("Estado de las colas de prioridad tras leer del broker IoT:");
  let msg="Expiration: "
  for (let i = 0; i < priorityMsgQueues.length; i++) {
    msg+="P"+(i+1)+":"+pirorityExpirationTimeQueue[i]+"ms ";
  }
  console.log(msg);
...
async function expirationMsgQueueHandler() {
  ...
  for (let i = 0; i < priorityMsgQueues.length; i++) {
    ...
    if (expiredCount > 0) console.log("Expired P",i+1,":",expiredCount,"(exp.", pirorityExpirationTimeQueue[i], " ms.) - expQueue: ", expirationMsgQueue.length, "msg.");
  }
